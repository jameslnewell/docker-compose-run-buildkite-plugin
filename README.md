...
2. **Run** — starts the target service's dependency tree (waiting for their conditions) and runs the target
...
1. **Pull Phase** — When Compose supports both `pull --include-deps` and `run --pull`, runs `docker compose pull --include-deps <service>` to fetch only the target service and its dependency tree. Skipped on older Compose that lacks these flags.
2. **Run Phase** — Executes `docker compose run --rm <service>` with any configured overrides. Without `--no-deps`, Compose brings up the service's `depends_on` tree, waits for their conditions, then runs the target. Passes `--pull never` when the pull phase already fetched the images.
3. **Cleanup Phase** — Always runs `docker compose down --volumes --remove-orphans` and collects logs as artifacts
